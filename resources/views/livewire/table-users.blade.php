<div class="flex flex-col ">
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            ...
            <div class="flex justify-between sm:ms-6">
                ...
                <a href="{{ route('users.create') }}">
                    <button type="button"
                        class="px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Créer</button>
                </a>
                        ...
                        @foreach ($allUsers as $user)
                            <tr wire:key='{{ $user->id }}'>
                                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                    <div>
                                        <h2 class="font-medium text-gray-800 dark:text-white ">{{ $user->firstName }}
                                        </h2>
                                    </div>
                                </td>
                                <td class="px-12 py-4 text-sm font-medium whitespace-nowrap">
                                    <div>
                                        <h2 class="font-medium text-gray-800 dark:text-white ">{{ $user->lastName }}
                                        </h2>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                    <div>
                                        @if ($user->role->role_name == 'admin')
                                            <span class="flex w-fit items-center gap-2 bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">
                                                {{ $user->role->role_name }}
                                                <span>
                                        ...
                                        @elseif($user->role->role_name == 'chauffeur')
                                            <span class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">{{ $user->role->role_name }}</span>         
                                        @else
                                            <span class="bg-yellow-100 text-yellow-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">{{ $user->role->role_name }}</span>
                                        @endif
                                ...
                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                    ...
                                    <button type="button" wire:click="setDeletedUser(@js($user->id))"
    ...
    <x-modal name="confirm-account-deletion" focusable>
        ...
        <form method="post" action="{{ $targeted_user != null ? route('users.destroy', $targeted_user->id) : '' }}"
            class="p-6">
            @csrf
            @method('delete')
